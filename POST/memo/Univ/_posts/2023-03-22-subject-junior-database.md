...
* **외부/개념 사상(매핑)** : 논리적 데이터 독립성을 보장하며 응용 인터페이스라고 불린다.
* **개념/내부 사상(매핑)** : 물리적 데이터 독립성을 보장하며 저장 인터페이스라고 불린다.
...
* `질의 처리기(Query Processor)` : 사용자의 명령을 해석 
 ...
 - 트랜잭션 관리자 : 사용자 접근권한, 데이터 제약조건 등을 확인
* `저장 데이터 관리자(Stored Data Manager)` : 운영체제의 기능을 사용해 DB와 데이터 사전을 관리
...
`데이터베이스 시스템(DBS)`이란 **DBMS와 DB를 포함하는 개념**으로 DB에 데이터를 저장, 관리하고 필요한 정보를 생성해주는 시스템이다.

*정보시스템과 데이터베이스시스템의 차이는?

## [DB 사용자]

DB를 사용하는 사람들을 다음 세가지로 분류할 수 있다.
* **DB 관리자(DBA; DataBase Administrator)** : DB를 설계,구축,운영,관리하는 역할을 한다.
* **최종 사용자(end user)** : 데이터를 조작(삽입,삭제,수정,검색)하기 위해 DB를 사용하는 사람이다. 최종 사용자는 데이터 조작어를 사용하는 캐주얼 사용자와 GUI를 사용하는 초보 사용자로 나뉜다.
* **응용 프로그래머(Application Programmer)** : 응용 프로그램을 만들때 데이터 조작어를 사용한다.
...
`관계 데이터 모델`은 개체를 `릴레이션`이라는 테이블로 표현하는 논리적 데이터 모델이다.
...
* `관계 대수` : 절차언어, 폐쇄특성(연산의 결과는 릴레이션)
  ...
  - **순수 관계 연산자** : 
    * 셀렉트(σ조건식(R)) : R에서 조건식(>,<,≥,≤,=,≠,⋁,⋀,¬)을 만족하는 투플의 집합 반환
    * 프로젝트(π속성리스트(R)) : R에서 주어진 속성들의 값으로만 구성된 투플의 집합 반환
    * 조인(R⨝S) : 공통 속성을 이용해 릴레이션 R,S의 투플들을 연결해서 새로운 투플의 집합 반환
    * 디비전(R÷S) : 릴레이션 S의 투플과 관련있는 R의 투플들의 집합 반환
* `관계 해석` : 원하는 데이터가 무엇인지만 기술
  - **도메인 관계 해석** : 
...
`SQL`은 관계 DB를 위한 표준 질의어로 사용되는 언어이다. SQL은 비절차식 언어이며, 대화식이나 삽입 방식으로 사용할 수 있다. SQL 명령어의 종류는 다음과 같다.
* 데이터 정의어(DDL; Data Definition Language) : 테이블을 생성(`create table`), 변경(`alter table`), 삭제(`drop table`)
* 데이터 조작어(DML; Data Manipulation Language) : 테이블에 데이터를 삽입(), 수정, 삭제, 검색
...
```sql
create table 테이블_이름{
  속성_이름 데이터_타입 [not null] 기본값 
  primary key(속성_이름) --기본키
}
```
