...
\[중간고사]
1. 운영체제 종류
1. 운영체제의 역할과 목표(자원관리(효율성), 자원보호(안정성), 하드인터(확장성), 사용인터(편리성))
1. 최초의 컴퓨터 - 에니악
1. 분산시스템(정의,목적,원리)
1. p2p 예-카카오톡, 블록체인(완전한 p2p)
1. 커널 정의, 종류(마이크로구조커널?)
o
1. 컴퓨터의 구조
1. 폰노이만 구조(옛날 하드와이어링 방식과 비교해서)
1. 요리사 모형으로 컴퓨터 설명, 운영체제 작업 어디에 대응되는지
1. 버스의 종류, 뭐가더 빠른지
1. 프로그래밍 언어 발전된 부분,장점(컴파일), 인터프리터와 차이
1. 레지스터 종류
1. 버퍼란
1. 인터럽트-폴링 차이
0
1. 프로세스 상태(기본:생성상태,준비상태,실행상태,완료상태 + 추가:대기상태)
1. 컨텍스트 체인지(문맥교환 그림 시험에 나올거 같음, 교재 그림 3-14)
1. 운영체제 입장에서 작업은 프로세스, cpu입장에서는 스레드
1. 멀티 태스킹 멀티 스레드 등 차이 설명
1. 프로세스 구조에서 동적할당영역에 힙 스택 어떤 방식으로 채워지는지
...
* `보류상태` : 메모리에서 **스왑영역**으로 쫓겨나 작업을 '일시정지'한 상태. 메모리가 꽉 찰 때, 오류가 발생했을 때, 악의적인 공격을 한다고 판단되었을 때, 매우 긴 주기로 반복되는 프로세스인 경우, 대기상태에서 입출력이 지연될 때 보류상태로 전환됨. 보류상태는 재시작하여 다시 기존의 상태로 돌아갈 수 있음.
...
다음은 프로세스의 연산이다.
* `fork()` : **실행중인 프로세스를 복사**하는 시스템 호출이다. fork()가 실행되면 기존 프로세스를 복사하고 PPID와 CPID가 설정해 부모 자식관계를 설정한다. 기존의 프로세스와 복사된 프로세스의 차이는 PID, PPID, CPID이다. fork()의 장점은 프로세스의 생성 속도가 빠르고, 부모 프로세스의 자원을 추가 작업없이 상속받을 수 있고, 자식 프로세스 종료 후 사용중인 자원을 부모 프로세스가 정리할 수 있어 시스템 관리를 효율적으로 할 수 있다는 점이다. 
* `exac()` : 기존 프로세스를 새로운 프로세스로 전환하는 시스템 호출이다. 보통 fork와 같이 사용하여 새로운 프로세스를 만들고 exac()으로 내용을 바꾼다. exac()이 실행되면 프로세스의 코드영역, 데이터영역이 바뀌고, 스택영역, 프로그램 카운터 및 각종 레지스터 값이 모두 초기화된디. exac()의 장점은 프로세스의 구조를 재활용해 속도가 빠르고, fork와 연계했을 때 부모-자식 관계를 사용할 수 있어 시스템 관리를 효율적으로 할 수 있다는 것이다.
