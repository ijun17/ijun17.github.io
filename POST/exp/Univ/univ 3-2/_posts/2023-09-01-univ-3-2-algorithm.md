...
`점근적 표기법`
* **임의의 표기법 X에 대해 f(n) = X(g(n)), F = f(n)/g(n), n이 매우 클 때**
* 빅오 O(g(n)): F <= c (점근적 상한)
* 빅오메가 Ω(g(n)): F >= c (점근적 하한)
* 빅세타 Θ(g(n)): c1 <= F <= c2 (상하한) 
* 스몰오 o(g(n)): F < c (점근적 상한) 
* 스몰오메가 ω(g(n)): F > c (점근적 하한) 
...
* 메인 메모리가 모든 키 값을 수용할 수 없다면 외부검색트리를 사용해야 하는데 이때 디스크 접근 시간이 검색의 효율을 결정
...
treeSearch(t,x) 
* t가 NIL이거나 x면 t를 반환, 아니면 대소비교해서 왼쪽이나 오른쪽 자식에게 재귀로 호출
* O(log n) ~ O(n)
