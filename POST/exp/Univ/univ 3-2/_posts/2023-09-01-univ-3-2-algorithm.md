...
`점근적 표기법(임의의 표기법 X에 대해 f(n)=X(g(n))에 대해)`
* O(g(n)): 최대 g(n)의 비율로 증가하는 함수(집합) (점근적 상한) f <= c*g *ex) n = O(n^2)*
* Ω(g(n)): 최소 g(n)의 비율로 증가하는 함수(집합) (점근적 하한) f >= c*g *ex) n^3 = Ω(n^2)*
* Θ(g(n)): g(n)의 비율로 증가하는 함수(집합) (O와 Ω의 교집합) c1*g <= f <= c2*g *ex) n^2 = Ω(n^2)*
* o(g(n)): g(n)보다 느린 비율로 증가 (점근적 상한)
* ω(g(n)): g(n)보다 빠른 비율로 증가 (점근적 하한)
...
## 점화식의 점근적 분석 방법
* 반복 대치 : 이전 항으로 반복해서 대치해가며 마지막 n에 대한 식으로 치환하는 방법
* 추정후 증명 : 점근적 복잡도를 미리 추정한다음 귀납적으로 증명
* 마스터 정리
