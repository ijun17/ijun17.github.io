...
1 결정트리
* 선형 모형의 한계를 극복
* 의사결정트리 max_depth=1일때 > 분기 하나로만 예측
* sklearn.tree.plot_tree(predictor) 로 시각화

2 결정 트리 좋은 분할 직접 찾기
* 트리에서 위에 있는 피쳐가 가장 중요한 피쳐
* 좋은 yhat을 얻게 해주는 분할이 좋은 분할
* r2 스코어로 좋은 분할 판단
* 맥스뎁스가 1일 경우 적당한 c를 고르고 분할 함
* 뎁스가 2일 경우 뎁스 1의 결과로 발생한 분할을 각각 다시 분할
* 피쳐가 두개일경우 

3 의사결정트리 논쟁(선형 모델과 비교하여)
* 트리의 경우 이상치에 강함 / 선형 모델은 상대적으로 약함
* 교호작용을 고려하지않아도 됨
* 다중공선성 문제도 모형이 덜 망함: 중요한 피쳐가 위로 중요하지 않으면 아래로
* 필요없는 변수가 있어도 모형이 덜 망함
* 설명하기 쉽다, 특성들의 중요도 파악 쉬움
* 비선형성을 쉽게 모델링 > 잘 맞춘다
* 모형에 대한 가정이 필요없음(넌파라메틱 모형 특징)
* 단점: 오버피팅이 일어나기 쉽다 > 오버피팅을 잡기 위해 배깅, 랜덤포레스트, 부스팅 등장
* 현재 최고는 부스팅 계열의 XGBoost, LightGBM, CatBoost

* 트리 뎁스를 얼마나 할건가
* 지니 계수를 어떻게 할것인가

* 최소 샘플 분할: 노드를 분할하기 위한 최소 샘플 수
* 가지치기: 트리의 불필요한 부분제거
* 정보이듯: 분할 전후 엔트로피 차이 측정
* 지니 불순도: 노드의 순도 측정 지표
...
5 배깅
* 트리의 과적합을 해결
* 사이킷런의 앙상블의 배깅리그레서에 위치
* 샘플에서 몇개를 중복을 허용해서 뽑아서 적합(반복) > 만들어진 트리들로 예측을 시키고 평균을 구함
* 부스트랩으로 이러한 방법을 도출해내지 않았을까?
* 앙상블 : 오케스트라에서 합주 / 데이터 사이언스 분야에서는 모델의 다양성

6 랜덤포레스트
* 배깅의 단점을 해결하기 위해 등장
* 배깅의 단점: 트리가 다양하지 않게 나옴 높이별 판단 피쳐 종류가 거의 동일
* 랜덤으로 피쳐를 뽑아서 트리를 만듦(배깅+맥스피쳐)
* (주의)랜덤포레스트에서 맥스 피쳐스가 1.0(디폴트값)으로 지정하면 전체 속성을 뜻하는 거임(=배깅)

7 부스팅
* 의사결정나무에서 최고의 성능(가장 중요)
* 배깅과 랜덤포레스트가 다양성이었다면, 부스팅은 **성장**
* 처음에 약하게 학습 > 부족한 공부는 조금씩 강화하면서 보완
